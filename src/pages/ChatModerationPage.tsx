...
import { ChatMessage, Course, UserRole } from '../types';
...
export const ChatModerationPage = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (messagesData) {
          ...
          const formattedMessages = messagesData.map(message => {
            return {
              ...
              user: {
                ...
                role: toUserRole(message.users.role),
                created_at: message.users.created_at || new Date().toISOString(),
          ...
          setFilteredMessages(formattedMessages as ChatMessage[]);
        }
      } catch (err: any) {
  ...
  const approveMessage = async (id: string) => {
    try {
      ...
      addToast('Сообщение одобрено.', 'success');
    } catch (err: any) {
  ...
  const handleRejectClick = (messageId: string) => {
  ...
  return (
    <Layout>
                  ...
                  {needsModeration && (
                    <div className="px-4 py-2 bg-gray-50 border-t border-gray-100 flex justify-end space-x-2">
                      <Button 
                        ...
                        onClick={() => handleRejectClick(message.id)}
                      >
      ...
      <ConfirmationModal
        ...
        message="Это действие необратимо. Вы уверены, что хотите удалить это сообщение?"
        confirmText="Удалить"
        cancelText="Отмена"
